-- ============================================================================
-- OEE Categorization Fix: รองรับ BREAKDOWN และ CHANGEOVER ใน Availability calc
-- ----------------------------------------------------------------------------
-- ก่อนแก้:
--   • PLANNED, SETUP events → planned bucket
--   • UNPLANNED, PERFORMANCE_LOSS, NULL → unplanned bucket
--   • BREAKDOWN, CHANGEOVER → ตกหล่น ไม่ถูกนับใน OEE
--
-- หลังแก้ (ตามมาตรฐาน OEE/TPM):
--   Planned bucket (ไม่ลด Availability):
--     • PLANNED — Maintenance, Cleaning, Meeting ฯลฯ
--     • CHANGEOVER — Setup ระหว่างเปลี่ยน SKU/สูตร (Net OEE convention)
--     • SETUP events
--   Unplanned bucket (ลด Availability):
--     • UNPLANNED — รอวัตถุดิบ, รอคน
--     • BREAKDOWN — เครื่องเสีย
--     • PERFORMANCE_LOSS — minor stop
--     • NULL — กรณีไม่ระบุ reason
-- ============================================================================
...
BEGIN
    ...
    LOOP
        ...
        -- Planned downtime: PLANNED + CHANGEOVER + SETUP events
        SELECT COALESCE(SUM(EXTRACT(EPOCH FROM (COALESCE(pe.end_ts, now()) - pe.start_ts))/60), 0)::INT
         ...
         WHERE pe.shift_calendar_id = p_shift_calendar_id
           ...
           AND dr.category          IN ('PLANNED', 'CHANGEOVER');
        ...
        -- Unplanned: UNPLANNED, BREAKDOWN, PERFORMANCE_LOSS, NULL
        SELECT COALESCE(SUM(EXTRACT(EPOCH FROM (COALESCE(pe.end_ts, now()) - pe.start_ts))/60), 0)::INT
          INTO v_unplanned_dt
          FROM public.production_events pe
...
-- rpc_recalc_oee_for_shift: เหมือนกัน — รวม BREAKDOWN/CHANGEOVER เข้าสูตร
...
BEGIN
    ...
    LOOP
        ...
        -- Planned: PLANNED + CHANGEOVER + SETUP
        ...
        SELECT COALESCE(SUM(EXTRACT(EPOCH FROM (COALESCE(end_ts, now()) - start_ts)) / 60), 0)::INTEGER
          INTO v_setup_time
          FROM public.production_events
         WHERE machine_id = v_machine.id AND shift_calendar_id = p_shift_calendar_id AND event_type = 'SETUP';
