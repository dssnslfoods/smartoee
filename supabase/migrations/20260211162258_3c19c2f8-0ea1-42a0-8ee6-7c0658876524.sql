...
-- Create holidays table for pre-defined special holidays
CREATE TABLE public.holidays (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  company_id UUID NOT NULL REFERENCES public.companies(id),
  plant_id UUID REFERENCES public.plants(id),  -- NULL = all plants in company
  holiday_date DATE NOT NULL,
  name TEXT NOT NULL,
  description TEXT,
  is_recurring BOOLEAN NOT NULL DEFAULT false,  -- true = repeats annually (same month/day)
  created_by UUID,
...
-- RLS Policies
CREATE POLICY "Admins full access to holidays"
  ON public.holidays FOR ALL
  USING (is_admin(auth.uid()))
  WITH CHECK (is_admin(auth.uid()));

CREATE POLICY "Supervisors can manage holidays in their company"
  ON public.holidays FOR ALL
...
-- Update rpc_recalc_oee_for_shift to skip holidays
