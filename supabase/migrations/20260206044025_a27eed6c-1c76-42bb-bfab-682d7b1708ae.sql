...
-- Remove ideal_cycle_time_seconds from products table (now lives in production_standards)
ALTER TABLE public.products DROP COLUMN IF EXISTS ideal_cycle_time_seconds;

-- Update rpc_recalc_oee_for_shift: remove product fallback, use production_standards > machine default only
...
BEGIN
    ...
    LOOP
        -- Calculate weighted ideal cycle time from production_standards only
