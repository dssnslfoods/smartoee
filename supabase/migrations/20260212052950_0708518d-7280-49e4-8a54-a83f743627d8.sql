...
CREATE OR REPLACE FUNCTION public.rpc_recalc_oee_for_shift(p_shift_calendar_id uuid, p_force_working_day boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
    ...
    LOOP
        ...
        IF NOT v_has_events AND NOT v_has_counts THEN
            -- If force_working_day is true, create snapshot with Availability=0
            IF p_force_working_day THEN
                INSERT INTO public.oee_snapshots (
                    ...
                    0, 0
                ) ON CONFLICT (scope, scope_id, period, shift_calendar_id) 
                  DO UPDATE SET
                    availability = 0,
                    performance = 0,
                    quality = 0,
                    oee = 0,
                    run_time_minutes = 0,
                    downtime_minutes = v_planned_time,
