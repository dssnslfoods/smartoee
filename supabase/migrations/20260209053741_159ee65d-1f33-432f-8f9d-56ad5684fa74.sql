-- Fix rpc_start_event: prefer active shifts, and update fallback to filter by active shifts
-- This prevents the function from associating events with inactive/deactivated shifts
...
BEGIN
    ...
    -- Get current shift calendar - handle normal ACTIVE shifts (start < end)
    ...
    -- Handle overnight ACTIVE shifts (start > end, e.g., 22:00-06:00)
    ...
    -- Fallback: get any ACTIVE shift for today (allows operation outside exact shift hours)
    IF v_shift_calendar_id IS NULL THEN
        SELECT sc.id INTO v_shift_calendar_id
        FROM public.shift_calendar sc
        JOIN public.shifts s ON sc.shift_id = s.id
    ...
    -- Final fallback: any shift_calendar for today (even inactive shifts)
    IF v_shift_calendar_id IS NULL THEN
        SELECT sc.id INTO v_shift_calendar_id
        FROM public.shift_calendar sc
        WHERE sc.plant_id = v_machine.plant_id
...
-- Also fix rpc_add_counts to prefer active shifts
...
BEGIN
    ...
    -- Get current shift calendar - handle normal ACTIVE shifts
    SELECT sc.id INTO v_shift_calendar_id
    FROM public.shift_calendar sc
    JOIN public.shifts s ON sc.shift_id = s.id
    WHERE sc.plant_id = v_machine.plant_id
    ...
    -- Handle overnight ACTIVE shifts
    IF v_shift_calendar_id IS NULL THEN
        SELECT sc.id INTO v_shift_calendar_id
        FROM public.shift_calendar sc
        JOIN public.shifts s ON sc.shift_id = s.id
    ...
    -- Fallback: any ACTIVE shift for today
    IF v_shift_calendar_id IS NULL THEN
        SELECT sc.id INTO v_shift_calendar_id
        FROM public.shift_calendar sc
        JOIN public.shifts s ON sc.shift_id = s.id
    ...
    -- Final fallback: any shift_calendar for today
    IF v_shift_calendar_id IS NULL THEN
        SELECT sc.id INTO v_shift_calendar_id
        FROM public.shift_calendar sc
        WHERE sc.plant_id = v_machine.plant_id
