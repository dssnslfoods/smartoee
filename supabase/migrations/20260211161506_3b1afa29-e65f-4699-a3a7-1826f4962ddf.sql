...
BEGIN
    ...
    LOOP
        -- Check if this machine has any events or counts for this shift
        -- If not, treat as special holiday — skip OEE calculation
        SELECT EXISTS (
        ...
        IF NOT v_has_events AND NOT v_has_counts THEN
            -- No activity = special holiday, remove any existing snapshot and skip
        ...
        SELECT 
            COALESCE(
                SUM(
                    COALESCE(ps.ideal_cycle_time_seconds, v_machine.ideal_cycle_time_seconds) *
