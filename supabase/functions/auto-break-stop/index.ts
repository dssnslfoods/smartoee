...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: templates, error: tplErr } = await supabase
      ...
      .select("id, plant_id, shift_id, break_start_time, break_end_time")
      .eq("is_active", true)
    ...
    const activeBreaks = templates.filter((t) => {
      ...
      const breakEnd = t.break_end_time as string;

      // Check if now is within the first 2 minutes of break start
      // This prevents re-closing events that were manually restarted during break
        ...
        const { error: updateErr } = await supabase
          ...
          .update({
            ...
            notes: (ev.notes ? ev.notes + " | " : "") +
              `Auto-stopped: พักกลางวัน (${(tpl.break_start_time as string).slice(0, 5)}-${(tpl.break_end_time as string).slice(0, 5)})`,
          })
          .eq("id", ev.id)
          .is("end_ts", null);
